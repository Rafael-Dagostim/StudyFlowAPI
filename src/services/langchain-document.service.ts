...
export class LangChainDocumentProcessor {
  ...
  static validateFile(file: Express.Multer.File): {
    ...
    if (!this.isFileTypeSupported(file.originalname)) {
      return {
        isValid: false,
        error: `File type not supported by LangChain processor: ${path.extname(
          file.originalname
        )}`,
      };
    }
  ...
  static getFileProcessingInfo(file: Express.Multer.File) {
    ...
    return {
      ...
      processingMethod: isSupported
        ? "LangChain Native Loaders"
        : "Unsupported",
    };
  }
   ...
   */
  static async processMultipleDocuments(files: Express.Multer.File[]): Promise<
    Array<{
      filename: string;
      success: boolean;
      result?: {
  ...
  static getProcessingStats(
    ...
    const totalChunks = successful.reduce(
      (sum, r) => sum + (r.result?.chunks.length || 0),
      0
    );
    ...
    return {
      ...
      averageChunksPerFile:
        successful.length > 0 ? totalChunks / successful.length : 0,
  ...
  static analyzeDocument(text: string) {
    ...
    const estimatedLanguage =
      portugueseMatches > englishMatches ? "Portuguese" : "English";
